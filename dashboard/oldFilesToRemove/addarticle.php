...
<?php
  ...
  if (isset($_POST["submit"])) {
    ...
    try {
        $sql = "SELECT 	id FROM users WHERE name='".$author."'";
        $stmt = $conn->prepare($sql);
        $stmt->execute();
        $result = $stmt->setFetchMode(PDO::FETCH_ASSOC);
        foreach(new TableRows(new RecursiveArrayIterator($stmt->fetchAll())) as $k=>$v) {
    ...
    try {
      ...
      $sql = "INSERT INTO article(title, text, image,id_categorie,idUser) VALUES ('$title' , 'hgonrog', '$filename',$id_categorie,$id_author)";
